...
const ProductCard: React.FC<{ product: Product }> = ({ product }) => {
    ...
    const handleAddToCart = async (product: Product, e: React.MouseEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                showToast(`Gagal menambahkan ke keranjang: ${errorData.error}`, "error");
            }
    ...
    return (
        <motion.div
            ...
            whileHover={{ y: -5 }}
            onClick={() => navigate(`/user/selected/${product.id}`)}
            onMouseEnter={() => setIsHovered(true)}
            onMouseLeave={() => setIsHovered(false)}
            className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden relative
                    group cursor-pointer border border-transparent hover:border-green-500"
        >
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setTimeout(() => setIsLoading(false), 800); // Simulate loading for smoother transition
            }
        };
        fetchData();
    }, []);
    ...
    const filteredDataProduct = data
        ...
        .sort((a, b) => {
            switch (sortOption) {
                ...
                default: // latest
                    return 0; // Assuming the array is already in the latest order
            }
        });
